...
const STORAGE_STATE_PATH = path.resolve(AUTH_DIR, 'admin.json');
...
export default async function globalSetup(config: FullConfig): Promise<void> {
  ...
  await page.locator('#email').fill(process.env.ADMIN_SEED_EMAIL!);
  await page.locator('#password').fill(process.env.ADMIN_SEED_PASSWORD!);
  await Promise.all([
    page.waitForURL(/\/lms\/admin/, { timeout: 30_000 }),
    page.locator('button[type="submit"]').click(),
  ]);

  // Let the dashboard's initial bootstrap calls settle so the saved storage
  // state reflects a fully-authenticated, loaded session.
  await page.waitForSelector('text=Admin Panel', { timeout: 15_000 }).catch(() => {});
  await page.waitForTimeout(1000);

  await context.storageState({ path: STORAGE_STATE_PATH });
  await browser.close();

  console.log(`[global-setup] Logged in as the real admin, saved storage state to ${STORAGE_STATE_PATH}`);
}

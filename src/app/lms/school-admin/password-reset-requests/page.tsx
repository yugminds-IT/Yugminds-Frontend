...
export default function PasswordResetRequestsPage() {
  ...
  const loadRequests = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await schoolAdminApi.passwordResetRequests.list({ limit: 200 });
  ...
  const filtered = tabSource.filter((r) => {
    ...
    return !q || r.email.toLowerCase().includes(q) || (r.profiles?.full_name ?? "").toLowerCase().includes(q) || r.user_role.toLowerCase().includes(q) || (r.approved_by_name ?? "").toLowerCase().includes(q);
  });
    ...
    <div className="p-8 bg-white min-h-screen">
      ...
      <div className="flex gap-3 mb-5">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input placeholder="Search by email, name, role, approver…" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10" />
        </div>
        {activeTab === "history" && (

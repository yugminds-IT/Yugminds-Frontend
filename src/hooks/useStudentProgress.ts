...
export function useCourseProgressStats(courseId: string, userRole: "teacher" | "school-admin" | "admin") {
  return useQuery({
    queryKey: ["progress-stats", "course", userRole, courseId],
    queryFn: async () => {
      if (userRole === "teacher") {
        const res = await teacherApi.studentProgress.list({ course_id: courseId });
        return res.data as TeacherProgressResponse;
      }
      if (userRole === "school-admin") {
        const res = await schoolAdminApi.studentProgress.list({ course_id: courseId });
        return res.data as SchoolAdminProgressResponse;
      }
      const res = await adminApi.studentProgress.list({ course_id: courseId });
      return res.data as AdminProgressResponse;
    },
    enabled: !!courseId,
    staleTime: 30_000,
...
export function useGradeProgressStats(grade: string, userRole: "teacher" | "school-admin" | "admin") {
  return useQuery({
    queryKey: ["progress-stats", "grade", userRole, grade],
    queryFn: async () => {
      if (userRole === "teacher") {
        const res = await teacherApi.studentProgress.list({ grade });
        return res.data as TeacherProgressResponse;
      }
      if (userRole === "school-admin") {
        const res = await schoolAdminApi.studentProgress.list({ grade });
        return res.data as SchoolAdminProgressResponse;
      }
      const res = await adminApi.studentProgress.list({ grade });
      return res.data as AdminProgressResponse;
    },
    enabled: !!grade,
    staleTime: 30_000,
